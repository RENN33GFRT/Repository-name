...
from src import CVS_and_XLSX_reader, generators, processing, utils, widget, collections
...
3. Получить информацию о транзакциях из XLSX-файла"""
    ...
    if file_type == 1:
        ...
        file = utils.get_transactions(file_path)
    elif file_type == 2:
        file_path = os.path.join(data_dir, "transactions.csv")
        file = CVS_and_XLSX_reader.CSV_file_read(file_path)
        print(file)
    elif file_type == 3:
        file_path = os.path.join(data_dir, "transactions_excel.xlsx")
        file = CVS_and_XLSX_reader.XLSX_file_read(file_path)
    ...
    if word_sort.lower() in ["да"]:
        file = collections.description_filter(file, word)
